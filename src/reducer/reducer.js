...
export default function allActions(state=initalState, action) {
  const newHealth = Math.round(10 * Math.pow(state.monsterLevel, 1.5));
  switch (action.type) {
...
case ActionTypes.MORE_MONEY: {
  ...
  return{
    ...
    const newBalance = Math.round(
      state.heroCash + (state.healthMax * 0.05) + state.monsterLevel
    );
    console.log("New balace: " + newBalance);
    console.log("Gold gaiend: " + Math.round((state.healthMax * 0.05) + state.monsterLevel));
    return {
      ...state,
